...
const CampHUD: React.FC<CampHUDProps> = ({ 
    ...
    return (
        <>
            ...
            <div className={`absolute top-0 left-0 p-8 z-30 flex flex-col gap-4 items-start pointer-events-none ${uiFadeClass}`}>
                
                {/* Clicking level/dossier now opens Statistics (onOpenStats) */}
                <div className={`bg-slate-900/95 p-4 border-l-4 border-blue-500 shadow-2xl cursor-pointer pointer-events-auto hover:bg-slate-800 transition-colors w-[320px]`} onClick={onOpenStats}>
                    <h1 className="text-4xl font-black text-white tracking-tighter leading-none uppercase">{getRank(stats.level)}</h1>
