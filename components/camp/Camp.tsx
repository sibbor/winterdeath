...
import { WEAPONS, MAP_THEMES, FAMILY_MEMBERS, PLAYER_CHARACTER, CHATTER_LINES } from '../../constants';
...
const Camp: React.FC<CampProps> = ({ stats, currentLoadout, weaponLevels, onSaveStats, onSaveLoadout, onSelectMap, onStartMission, currentMap, debugMode, onToggleDebug, showFps, onToggleFps, familyMembersFound, isMapLoaded, bossesDefeated, onResetGame, hasCheckpoint, onFPSUpdate }) => {
  ...
  // Modal Close / Open Settings on Esc
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      frame++; activeRef.current = activeModal;
      const now = Date.now();
  ...
  }, [activeModal, familyMembersFound, debugMode, textures]); 
  ...
  return (
    <div className={`relative w-full h-full bg-black font-sans overflow-hidden`}>
      ...
      <div ref={chatOverlayRef} className="absolute inset-0 pointer-events-none z-40 overflow-hidden" />
