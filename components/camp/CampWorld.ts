...
export const CampWorld = {
    setupTerrain: (scene: THREE.Scene, textures: Textures) => {
        ...
        // Trees
        const treeGeo = new THREE.ConeGeometry(3, 18, 7);
        const trunkGeo = new THREE.CylinderGeometry(0.8, 1.2, 4);
        const trunkMat = new THREE.MeshStandardMaterial({ map: textures.wood, color: 0x332211, roughness: 0.9 });
        const leavesMat = new THREE.MeshStandardMaterial({ map: textures.pine, color: 0x888888, roughness: 0.8, bumpMap: textures.pine, bumpScale: 0.2 });

        const spawnTree = (x: number, z: number, scale: number) => {
            const g = new THREE.Group();
            g.position.set(x, 0, z);
            const t = new THREE.Mesh(trunkGeo, trunkMat); t.position.y = 2; t.castShadow = true; g.add(t);
            const l = new THREE.Mesh(treeGeo, leavesMat); l.position.y = 10; l.castShadow = true; g.add(l);
            g.scale.setScalar(scale); g.rotation.y = Math.random() * Math.PI;
            scene.add(g);
        };
