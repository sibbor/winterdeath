...
export const checkIsMobileDevice = (): boolean => {
    ...
    // Check for iPads/iPhones reporting as 'MacIntel' (Desktop Mode)
    const isMockMac = navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1;
    ...
    // We only consider it mobile if it's a mobile UA, OR if it's a small screen WITH touch capability, OR if it's an iOS device in desktop mode
    return uaCheck || isMockMac || (widthCheck && touchCheck);
};
