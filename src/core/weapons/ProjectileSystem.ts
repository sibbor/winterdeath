...
export interface Projectile {
    ...
    weapon: string;
    vel: THREE.Vector3;
    origin: THREE.Vector3; // Track origin for distance calculations
    damage: number;
    life: number;
    maxRadius?: number; // For throwables
    marker?: THREE.Mesh; // For throwables
    hitEntities?: Set<string>; // For piercing weapons to track hits
}
...
export const ProjectileSystem = {
    ...
    spawnBullet: (scene: THREE.Scene, origin: THREE.Vector3, dir: THREE.Vector3, weapon: string, damage: number) => {
        ...
        ProjectileSystem.projectiles.push({
            ...
            type: 'bullet',
            weapon: weapon,
            vel: dir.multiplyScalar(visuals.speed),
            origin: origin.clone(),
            damage: damage,
            life: 1.5,
            hitEntities: new Set()
    ...
    spawnThrowable: (scene: THREE.Scene, origin: THREE.Vector3, dir: THREE.Vector3, throwableId: string, chargeRatio: number) => {
        ...
        if (!def) return; // Unknown throwable
        ...
        const targetPos = origin.clone().add(dir.normalize().multiplyScalar(throwDist));
        ...
        ProjectileSystem.projectiles.push({
            ...
            origin: origin.clone(),
            damage: 0, // Damage handled in onImpact
            life: timeToTarget + 0.5, // Safety buffer
    ...
    update: (delta: number, now: number, ctx: Omit<GameContext, 'addFireZone'>) => {
        ...
        // --- UPDATE FIRE ZONES ---
        for (let i = ProjectileSystem.fireZones.length - 1; i >= 0; i--) {
            ...
            if (fz.life <= 0) {
                ...
                ProjectileSystem.fireZones.splice(i, 1);
            }
        }
    },

    clear: (scene: THREE.Scene) => {
        ProjectileSystem.projectiles.forEach(p => { scene.remove(p.mesh); if (p.marker) scene.remove(p.marker); });
        ProjectileSystem.fireZones.forEach(f => scene.remove(f.mesh));
        ProjectileSystem.projectiles = [];
        ProjectileSystem.fireZones = [];
    }
};
...
function updateBullet(p: Projectile, index: number, delta: number, ctx: GameContext) {
    ...
    if (destroy || p.life <= 0) {
        ...
        ProjectileSystem.projectiles.splice(index, 1);
    }
}
...
function updateThrowable(p: Projectile, index: number, delta: number, ctx: GameContext, now: number) {
    ...
    if (p.mesh.position.y <= 0.2) {
        ...
        // Impact sound (explosion) is handled in onImpact for grenades/molotovs
        // We could play a generic thud if it was a dud, but all current throwables explode.
