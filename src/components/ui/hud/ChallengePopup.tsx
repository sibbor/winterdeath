...
const ChallengePopup: React.FC<ChallengePopupProps> = ({ onOpenAdventureLog }) => {
    ...
    // Tier colors: 1: Bronze (Orange), 2: Silver (Gray/Cyan), 3: Gold (Yellow)
    const tierColors = ['border-orange-500 text-orange-500', 'border-cyan-300 text-cyan-200', 'border-yellow-400 text-yellow-400'];
    const currentTierColor = tierColors[tier - 1] || 'border-yellow-500 text-yellow-500';

    return (
        <div
            key={activeChallenge.timestamp}
            className="fixed top-1/4 right-8 z-[100] pointer-events-auto animate-challengePop"
            onClick={handleInteraction}
        >
            <div className={`bg-black/95 border-l-4 ${currentTierColor.split(' ')[0]} p-5 shadow-[0_15px_50px_rgba(0,0,0,0.9)] backdrop-blur-md hud-gritty-base min-w-[300px] overflow-hidden cursor-pointer hover:bg-zinc-900/40 transition-colors`}>
                {/* Header */}
                <div className="flex flex-col relative z-10">
                    <span className={`text-[11px] font-black uppercase tracking-[0.4em] leading-none mb-2 opacity-80 ${currentTierColor.split(' ')[1]}`}>
                    ...
                    <div className="flex items-baseline gap-2">
                        ...
                        <span className={`text-xs font-bold uppercase tracking-widest ${currentTierColor.split(' ')[1]}`}>
                    ...
                    <p className="text-xs text-white/70 font-medium leading-relaxed italic mb-3">
                        {/* Criteria met display */}
                        {t(def.descriptionKey).replace('{target}', target.toLocaleString())}
                    </p>
                    ...
                    <div className="flex items-center justify-between mt-1">
                        ...
                        <div className="flex items-center gap-1.5 opacity-60">
                            <div className="px-1.5 py-0.5 border border-white/30 rounded text-[9px] font-mono font-bold text-white uppercase">
                                {isMobileDevice ? t('ui.tap') : 'ENTER'}
                            </div>
                            <span className="text-[9px] font-bold text-white/40 uppercase tracking-widest">{t('ui.view_details')}</span>
                        </div>
                ...
                <div className={`absolute bottom-0 left-0 h-1 bg-gradient-to-r from-transparent via-${currentTierColor.split('-')[1]}-500 to-transparent w-full opacity-50`} />
            </div>

            <style>{`
                @keyframes challengePop {
                    0% { opacity: 0; transform: translateX(50px) skewX(-10deg); filter: blur(10px); }
                    10% { opacity: 1; transform: translateX(0) skewX(0); filter: blur(0px); }
                    90% { opacity: 1; transform: translateX(0); }
                    100% { opacity: 0; transform: translateX(20px) translateY(-10px); filter: blur(5px); }
                }
