        ...
        switch (obj.type) {
            ...
            default:
                ...
                if (Object.keys(ObjectGenerator).includes(obj.type) || ['WallSection', 'Corner', 'DoorFrame', 'WindowFrame', 'Floor'].includes(obj.type)) {
                    baseCode = `        const obj_${obj.id} = SectorBuilder.spawnBuildingPiece(ctx, '${obj.type}', ${x.toFixed(2)}, ${z.toFixed(2)}, ${rotY.toFixed(2)});`;
                }
...
export const ${safeName}: SectorDef = {
    ...
    playerSpawn: { x: ${sector.spawns.player.x.toFixed(2)}, z: ${sector.spawns.player.z.toFixed(2)}, rot: ${sector.spawns.player.rot.toFixed(2)} },
    familySpawn: { x: ${sector.spawns.family.x.toFixed(2)}, z: ${sector.spawns.family.z.toFixed(2)} },
    bossSpawn: { x: ${sector.spawns.boss.x.toFixed(2)}, z: ${sector.spawns.boss.z.toFixed(2)} },
