...
export const Sector1: SectorDef = {
    ...
    generate: (ctx: SectorContext) => {
        ...
        // Decoration Trees
        for(let i=0; i<300; i++) {
            const xRaw = (Math.random()-0.5)*500; const zRaw = -Math.random()*600;
            if (xRaw > 150 && xRaw < 300 && zRaw < -250 && zRaw > -500) continue; 
            const p = {x: xRaw, z: zRaw};
            const t = new THREE.Group(); t.position.set(p.x, 0, p.z);
            const tr = new THREE.Mesh(GEOMETRY.treeTrunk, MATERIALS.treeTrunk); tr.position.y = 2; t.add(tr);
            const l = new THREE.Mesh(GEOMETRY.treeLeaves, MATERIALS.treeLeaves); l.position.y = 8; t.add(l);
            t.scale.setScalar(0.8 + Math.random()); scene.add(t); 
            obstacles.push({mesh: t, collider: { type: 'sphere', radius: 2 }});
        }
